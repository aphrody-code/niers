...
color_token!(
    ...
    ABYSS_BACKGROUND, "jeu-fond-abysse", 0.1963, 0.0242, 280.23, "#131420", "nuit", 2,
    "le fond le plus profond"
);
color_token!(
    "Un panneau sombre. Mesuré `#152d52` — teinte azur (2 % de l'atlas source).",
    NIGHT_BACKGROUND, "jeu-fond-nuit", 0.3000, 0.0726, 258.02, "#152d52", "azur", 2,
    "un panneau sombre"
);
color_token!(
    "Une surface bleue. Mesuré `#224271` — teinte azur (2 % de l'atlas source).",
    DEEP_BACKGROUND, "jeu-fond-profond", 0.3800, 0.0888, 258.02, "#224271", "azur", 2,
    "une surface bleue"
);
color_token!(
    "Une surface bleue active. Mesuré `#34588d` — teinte azur (2 % de l'atlas source).",
    MEDIUM_BACKGROUND, "jeu-fond-moyen", 0.4600, 0.0968, 258.02, "#34588d", "azur", 2,
...
color_token!(
    ...
    ICE_SURFACE, "jeu-surface-glace", 0.9400, 0.0282, 258.02, "#e0ecff", "azur", 2,
    "une carte claire"
);
color_token!(
    "Un dégradé clair, teinté du bleu de la tenue. Mesuré `#c7d9f3` — teinte azur (2 %).",
    MIST_SURFACE, "jeu-surface-brume", 0.8800, 0.0403, 258.02, "#c7d9f3", "azur", 2,
    "un dégradé clair, teinté du bleu de la tenue"
);
color_token!(
    "Un fond neutre. Mesuré `#e3d5c5` — teinte sable (13 % de l'atlas source).",
    CHALK_SURFACE, "jeu-surface-craie", 0.8800, 0.0263, 70.41, "#e3d5c5", "sable", 13,
    "un fond neutre"
);
color_token!(
    "Un texte secondaire. Mesuré `#b3988b` — teinte taupe (11 % de l'atlas source).",
    ASH_SURFACE, "jeu-surface-cendre", 0.7000, 0.0374, 46.43, "#b3988b", "taupe", 11,
    "un texte secondaire"
);
color_token!(
    "Une nuance douce. Mesuré `#9e8296` — teinte mauve (9 % de l'atlas source).",
    PINK_SURFACE, "jeu-surface-rose", 0.6400, 0.0435, 337.07, "#9e8296", "mauve", 9,
    "une nuance douce"
);
...
color_token!(
    ...
    VIVID_TEXT, "jeu-texte-vif", 0.9850, 0.0035, 59.65, "#fcfaf8", "creme", 25,
    "le texte sur fond sombre"
);
color_token!(
    "Un lien, un texte de second plan. Mesuré `#638dcb` — teinte azur (2 % de l'atlas).",
    SOFT_TEXT, "jeu-texte-doux", 0.6400, 0.1049, 258.02, "#638dcb", "azur", 2,
...
pub fn hex_to_rgb_triplet(hex: &str) -> [u8; 3] {
    let digits = hex.strip_prefix('#').unwrap_or_else(|| panic!("« {hex} » n'a pas de préfixe #"));
    assert!(digits.len() == 6, "« {hex} » n'a pas 6 chiffres hexadécimaux");
    let byte = |i: usize| u8::from_str_radix(&digits[i..i + 2], 16).unwrap_or_else(|e| panic!("« {hex} » : {e}"));
...
mod tests {
    ...
    fn hex_to_rgb_triplet_convertit_les_deux_couleurs_reellement_utilisees_par_lelevation() {
        ...
        assert_eq!(hex_to_rgb_triplet(super::ABYSS_BACKGROUND.hex), [0x13, 0x14, 0x20]);
    ...
    fn les_couleurs_du_jeu_suivent_le_calcul_reel_de_nie_aphrody() {
        for jeton in GAME_COLORS {
            let role = nie_aphrody::design::role(jeton.name).unwrap_or_else(|| {
                panic!("{} : absent de nie_aphrody::design (rôle renommé ou supprimé ?)", jeton.name)
            });
