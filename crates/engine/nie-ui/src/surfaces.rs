...
screen_color!(
    ...
    HEADER_BLUE, "screen-header-blue", 0.5912, 0.2241, 258.91, "#0874FF",
    "options.png", (700, 40, 1701, 81), 3, 35.61, "la barre de titre"
);
screen_color!(
    "Le bleu profond des rayures diagonales de la barre. Même recadrage, 2e classe (34.21 %).",
    HEADER_BLUE_DEEP, "screen-header-blue-deep", 0.5523, 0.2353, 261.08, "#0663F8",
    "options.png", (700, 40, 1701, 81), 3, 34.21, "les rayures diagonales de la barre de titre"
);
screen_color!(
    "Le gris de la tuile d'icône à gauche de la barre. `--crop 0,0,151,151 --k 4` → #A6A6A6 (30.83 %), le blanc du glyphe en 1re classe.",
    HEADER_ICON_GREY, "screen-header-icon-grey", 0.7252, 0.0000, 180.00, "#A6A6A6",
    "options.png", (0, 0, 151, 151), 4, 30.83, "la tuile d'icône à gauche de la barre de titre"
);
...
screen_color!(
    ...
    TAB_KEY, "screen-tab-key", 0.4239, 0.0000, 21.80, "#4E4E4E",
    "options.png", (700, 265, 161, 56), 3, 95.42, "les touches clavier aux bouts du bandeau d'onglets"
);
screen_color!(
    "Le bleu clair du milieu du bandeau. `--crop 900,265,141,56 --k 3` → #6CA8F0 (100 %).",
    TAB_LIGHT, "screen-tab-light", 0.7206, 0.1229, 253.81, "#6CA8F0",
    "options.png", (900, 265, 141, 56), 3, 100.0, "le fond clair du bandeau d'onglets"
);
screen_color!(
    "Le bleu saturé de l'onglet actif. `--crop 1065,265,111,56 --k 3` → #0149FF (83.17 %), le glyphe blanc en 2e classe.",
    TAB_ACTIVE, "screen-tab-active", 0.5141, 0.2718, 263.49, "#0149FF",
    "options.png", (1065, 265, 111, 56), 3, 83.17, "l'onglet actif"
);

// --- Lignes de réglage (options.png, controls.png) --------------------------------------------
screen_color!(
    "Le blanc d'une ligne au repos. `--crop 520,515,881,51 --k 3` → #FEFFFF (88.23 %).",
    ROW_WHITE, "screen-row-white", 0.9992, 0.0011, 197.15, "#FEFFFF",
    "options.png", (520, 515, 881, 51), 3, 88.23, "une ligne de réglage au repos"
);
screen_color!(
    "Le gris du libellé d'une ligne au repos. Même recadrage, 2e classe → #79797A (8.59 %).",
    ROW_LABEL, "screen-row-label", 0.5764, 0.0015, 286.35, "#79797A",
    "options.png", (520, 515, 881, 51), 3, 8.59, "le libellé d'une ligne au repos"
);
screen_color!(
    ...
    ROW_FOCUS, "screen-row-focus", 0.5987, 0.2200, 257.83, "#0078FF",
    "options.png", (600, 415, 501, 11), 3, 98.93, "la ligne de réglage focalisée (aplat, pas de dégradé vertical)"
);
screen_color!(
    ...
    ROW_VALUE_STRIP, "screen-row-value-strip", 0.8719, 0.0729, 236.34, "#A7DDFF",
    "controls.png", (1380, 420, 381, 51), 4, 92.60, "la colonne de valeur d'une ligne au repos"
);
screen_color!(
    "La colonne de valeur d'une ligne focalisée. `--crop 1380,1010,381,51 --k 4` → #08B5FF (94.39 %).",
    ROW_VALUE_STRIP_ACTIVE, "screen-row-value-strip-active", 0.7327, 0.1590, 237.05, "#08B5FF",
    "controls.png", (1380, 1010, 381, 51), 4, 94.39, "la colonne de valeur d'une ligne focalisée"
);
screen_color!(
    "Le bleu du texte de valeur (« Normal »). `--crop 1700,522,121,39 --k 4` → #5084B7 (23.33 %), le fond en 1re classe.",
    ROW_VALUE_TEXT, "screen-row-value-text", 0.5988, 0.0963, 249.27, "#5084B7",
    "options.png", (1700, 522, 121, 39), 4, 23.33, "le texte de la colonne de valeur"
);
screen_color!(
    "Le carré arrondi des flèches « < » « > » de la ligne focalisée. `--crop 1470,416,51,52 --k 4` → #0350E6 (62.97 %).",
    ROW_ARROW, "screen-row-arrow", 0.5008, 0.2353, 262.40, "#0350E6",
    "options.png", (1470, 416, 51, 52), 4, 62.97, "le carré des flèches de la ligne focalisée"
);
screen_color!(
    "Le liseré clair du carré de flèche. Même recadrage, 2e classe → #4782F7 (21.08 %).",
    ROW_ARROW_LIGHT, "screen-row-arrow-light", 0.6281, 0.1859, 262.29, "#4782F7",
    "options.png", (1470, 416, 51, 52), 4, 21.08, "le liseré du carré de flèche"
);
screen_color!(
    "Le bleu-gris du titre de section (« Paramètres du jeu »). `--crop 1133,352,295,33 --k 4` → #4B6D97 (26.21 %).",
    SECTION_TITLE, "screen-section-title", 0.5270, 0.0774, 254.25, "#4B6D97",
    "options.png", (1133, 352, 295, 33), 4, 26.21, "le titre de section au-dessus d'une liste"
);
screen_color!(
    "Le blanc de la barre de défilement. `--crop 2112,403,14,368 --k 3` → #FFFFFF (81.62 %).",
    SCROLLBAR, "screen-scrollbar", 1.0000, 0.0000, 90.00, "#FFFFFF",
    "options.png", (2112, 403, 14, 368), 3, 81.62, "le curseur de la barre de défilement"
);
screen_color!(
    "Le gris-bleu de la piste de défilement. Même recadrage, 2e classe → #96AABB (10.79 %).",
    SCROLLBAR_TRACK, "screen-scrollbar-track", 0.7282, 0.0333, 243.86, "#96AABB",
    "options.png", (2112, 403, 14, 368), 3, 10.79, "la piste de la barre de défilement"
);
...
screen_color!(
    ...
    DESCRIPTION_GREY, "screen-description-grey", 0.5328, 0.0284, 252.08, "#616E7D",
    "options.png", (100, 1235, 401, 71), 3, 66.94, "la barre de description en bas d'écran"
);
screen_color!(
    "Le gris foncé d'une touche (« V »). `--crop 915,1350,41,41 --k 3` → #4A4949 (47.83 %).",
    KEY_CAP, "screen-key-cap", 0.4064, 0.0013, 17.21, "#4A4949",
    "options.png", (915, 1350, 41, 41), 3, 47.83, "une touche clavier (fond)"
);
screen_color!(
    "Le gris du compteur « 13/13 ». `--crop 1780,1200,51,41 --k 3` → #545454 (79.29 %).",
    COUNT_BADGE, "screen-count-badge", 0.4459, 0.0000, 180.00, "#545454",
    "filters_elements.png", (1780, 1200, 51, 41), 3, 79.29, "la pastille de compteur"
);
screen_color!(
    "Le fond pâle du canevas. `--crop 128,384,257,257 --k 3` → #E0FAFF (54.19 %).",
    CANVAS_PALE, "screen-canvas-pale", 0.9678, 0.0279, 210.53, "#E0FAFF",
    "options.png", (128, 384, 257, 257), 3, 54.19, "le fond pâle du canevas"
);

// --- Le curseur (options.png) ----------------------------------------------------------------
screen_color!(
    "Le jaune-vert du curseur triangulaire. `--crop 455,405,71,76 --k 6` → #B5FF6B (29.19 %).",
    CURSOR_YELLOW, "screen-cursor-yellow", 0.9218, 0.1938, 131.54, "#B5FF6B",
    "options.png", (455, 405, 71, 76), 6, 29.19, "la bande jaune du curseur"
);
screen_color!(
    "Le vert du curseur. Même recadrage, 2e classe → #00CE87 (26.30 %).",
    CURSOR_GREEN, "screen-cursor-green", 0.7510, 0.1709, 159.65, "#00CE87",
    "options.png", (455, 405, 71, 76), 6, 26.30, "la bande verte du curseur"
);
screen_color!(
    "Le cyan pâle du curseur. Même recadrage, 4e classe → #CDF9FF (9.17 %) — la 3e est le bleu de la ligne derrière.",
    CURSOR_CYAN, "screen-cursor-cyan", 0.9532, 0.0457, 206.57, "#CDF9FF",
    "options.png", (455, 405, 71, 76), 6, 9.17, "la bande cyan du curseur"
);

// --- Le panneau FILTRES (filters_elements.png) ----------------------------------------------
screen_color!(
    "Le bleu royal du haut du panneau. `--crop 900,160,1151,26 --k 3` → #0048B9 (84.62 %).",
    PANEL_TOP, "screen-panel-top", 0.4431, 0.1890, 260.78, "#0048B9",
    "filters_elements.png", (900, 160, 1151, 26), 3, 84.62, "le haut du panneau externe"
);
screen_color!(
    ...
    PANEL_BOTTOM, "screen-panel-bottom", 0.3411, 0.1862, 263.67, "#002496",
    "filters_elements.png", (512, 1262, 1537, 26), 3, 82.54, "le bas du panneau externe"
);
screen_color!(
    "Le bleu nuit du corps interne. `--crop 540,705,741,321 --k 4` → #001E73 (68.85 %).",
    PANEL_BODY, "screen-panel-body", 0.2896, 0.1484, 263.16, "#001E73",
    "filters_elements.png", (540, 705, 741, 321), 4, 68.85, "le corps interne du panneau"
);
screen_color!(
    "Les glyphes en filigrane du corps. Même recadrage, 2e classe → #163181 (29.60 %).",
    PANEL_WATERMARK, "screen-panel-watermark", 0.3487, 0.1389, 265.33, "#163181",
    "filters_elements.png", (540, 705, 741, 321), 4, 29.60, "les glyphes en filigrane du corps"
);
screen_color!(
    "Le bleu clair des lettres du titre « FILTRES ». `--crop 493,166,283,72 --k 4` → #4183F4 (24.11 %), le panneau en 1re classe.",
    PANEL_TITLE, "screen-panel-title", 0.6259, 0.1826, 260.40, "#4183F4",
    "filters_elements.png", (493, 166, 283, 72), 4, 24.11, "les lettres du titre du panneau"
);

// --- Cases à cocher et pastilles d'icône (filters_elements.png) ------------------------------
screen_color!(
    "Le cyan de la coche. `--crop 636,386,52,50 --k 4` → #45FFF8 (38.00 %).",
    CHECK_CYAN, "screen-check-cyan", 0.9094, 0.1434, 191.10, "#45FFF8",
    "filters_elements.png", (636, 386, 52, 50), 4, 38.00, "la coche"
);
screen_color!(
    "Le bleu nuit de la case. Même recadrage, 2e classe → #012075 (32.58 %).",
    CHECK_BOX, "screen-check-box", 0.2960, 0.1484, 263.25, "#012075",
    "filters_elements.png", (636, 386, 52, 50), 4, 32.58, "le fond de la case à cocher"
);
screen_color!(
    "Le liseré de la case. Même recadrage, 3e classe → #2B62B6 (26.08 %).",
    CHECK_BOX_EDGE, "screen-check-box-edge", 0.5053, 0.1453, 258.85, "#2B62B6",
    "filters_elements.png", (636, 386, 52, 50), 4, 26.08, "le liseré de la case à cocher"
);
screen_color!(
    "La pastille Vent. `--crop 723,476,65,63 --k 3` → #8ED5FF (71.67 %).",
    CHIP_WIND, "screen-chip-wind", 0.8412, 0.0923, 235.38, "#8ED5FF",
    "filters_elements.png", (723, 476, 65, 63), 3, 71.67, "la pastille d'icône Vent"
);
screen_color!(
    "La pastille Feu. `--crop 723,570,65,62 --k 3` → #FF7155 (69.03 %).",
    CHIP_FIRE, "screen-chip-fire", 0.7160, 0.1791, 32.86, "#FF7155",
    "filters_elements.png", (723, 570, 65, 62), 3, 69.03, "la pastille d'icône Feu"
);
screen_color!(
    "La pastille Forêt. `--crop 1459,476,65,63 --k 3` → #ABFF38 (72.70 %).",
    CHIP_FOREST, "screen-chip-forest", 0.9124, 0.2308, 130.66, "#ABFF38",
    "filters_elements.png", (1459, 476, 65, 63), 3, 72.70, "la pastille d'icône Forêt"
);
screen_color!(
    "La pastille Montagne. `--crop 1459,570,65,62 --k 3` → #FFB936 (72.83 %).",
    CHIP_MOUNTAIN, "screen-chip-mountain", 0.8307, 0.1577, 78.07, "#FFB936",
    "filters_elements.png", (1459, 570, 65, 62), 3, 72.83, "la pastille d'icône Montagne"
);
...
screen_color!(
    ...
    BUTTON_PRIMARY, "screen-button-primary", 0.6779, 0.1797, 247.35, "#009DFF",
    "filters_elements.png", (1400, 1170, 101, 71), 3, 42.57, "le bouton principal (Confirmer)"
);
screen_color!(
    "Le bleu royal du bouton Réinitialiser. `--crop 850,1190,31,41 --k 3` → #3672E5 (100 %).",
    BUTTON_SECONDARY, "screen-button-secondary", 0.5765, 0.1859, 261.77, "#3672E5",
    "filters_elements.png", (850, 1190, 31, 41), 3, 100.0, "le bouton secondaire (Réinitialiser)"
);
screen_color!(
    "Le bleu sombre du bouton « Tout » dans le panneau. `--crop 1900,1050,81,56 --k 3` → #0030A2 (100 %).",
    BUTTON_TERTIARY, "screen-button-tertiary", 0.3748, 0.1881, 262.96, "#0030A2",
    "filters_elements.png", (1900, 1050, 81, 56), 3, 100.0, "un bouton posé sur le panneau"
);
screen_color!(
    "Le cyan du texte d'indice « Chercher par nom de joueur ». `--crop 1625,1350,514,41 --k 4` → #0EFFF1 (24.17 %), le fond en 1re classe.",
    SEARCH_HINT, "screen-search-hint", 0.9013, 0.1559, 187.70, "#0EFFF1",
    "filters_elements.png", (1625, 1350, 514, 41), 4, 24.17, "le texte d'indice cyan de la barre du bas"
);
...
screen_color!(
    ...
    TILE_DEEP, "screen-tile-deep", 0.3249, 0.1103, 258.79, "#09316B",
    "main_menu.png", (1062, 768, 245, 167), 6, 27.12, "le bleu profond d'une tuile du menu"
);
screen_color!(
    "Le bleu moyen d'une tuile. Même recadrage, 3e classe → #245293 (14.56 %).",
    TILE_MID, "screen-tile-mid", 0.4422, 0.1182, 257.59, "#245293",
    "main_menu.png", (1062, 768, 245, 167), 6, 14.56, "le bleu moyen d'une tuile du menu"
);
screen_color!(
    "Le bleu clair d'une tuile. Même recadrage, 4e classe → #4077C0 (14.38 %).",
    TILE_LIGHT, "screen-tile-light", 0.5668, 0.1277, 256.22, "#4077C0",
    "main_menu.png", (1062, 768, 245, 167), 6, 14.38, "le bleu clair d'une tuile du menu"
);
screen_color!(
    "Le biseau clair sous une tuile. `--crop 1062,928,245,15 --k 3` → #EAEDEC (63.54 %).",
    TILE_BEVEL, "screen-tile-bevel", 0.9435, 0.0035, 174.48, "#EAEDEC",
    "main_menu.png", (1062, 928, 245, 15), 3, 63.54, "le biseau clair sous une tuile"
);
screen_color!(
    "Le bleu nuit du pied de la fenêtre Informations. `--crop 500,232,77,49 --k 3` → #374469 (100 %).",
    INFO_FOOTER, "screen-info-footer", 0.3928, 0.0653, 268.56, "#374469",
    "main_menu.png", (500, 232, 77, 49), 3, 100.0, "le pied de la fenêtre Informations"
);

/// Les 45 couleurs `--screen-*`, dans l'ordre du bloc `:root` de `game-screens.css`.
pub const SCREEN_COLORS: [ScreenColor; 45] = [
    HEADER_BLUE, HEADER_BLUE_DEEP, HEADER_ICON_GREY,
    TAB_KEY, TAB_LIGHT, TAB_ACTIVE,
    ROW_WHITE, ROW_LABEL, ROW_FOCUS, ROW_VALUE_STRIP, ROW_VALUE_STRIP_ACTIVE, ROW_VALUE_TEXT,
    ROW_ARROW, ROW_ARROW_LIGHT, SECTION_TITLE, SCROLLBAR, SCROLLBAR_TRACK,
    DESCRIPTION_GREY, KEY_CAP, COUNT_BADGE, CANVAS_PALE,
    CURSOR_YELLOW, CURSOR_GREEN, CURSOR_CYAN,
    PANEL_TOP, PANEL_BOTTOM, PANEL_BODY, PANEL_WATERMARK, PANEL_TITLE,
    CHECK_CYAN, CHECK_BOX, CHECK_BOX_EDGE, CHIP_WIND, CHIP_FIRE, CHIP_FOREST, CHIP_MOUNTAIN,
    BUTTON_PRIMARY, BUTTON_SECONDARY, BUTTON_TERTIARY, SEARCH_HINT,
    TILE_DEEP, TILE_MID, TILE_LIGHT, TILE_BEVEL, INFO_FOOTER,
];
...
pub const SCREEN_LENGTHS: [(&str, &str, &str); 5] = [
    ("screen-header-height", "80px", "options.png : la barre couvre y 0..160 (160 px / 2)"),
    ("screen-tab-height", "32px", "options.png : boite ajustee de l'onglet actif y 262..324 (63 px / 2)"),
    ("screen-row-height", "40px", "controls.png : boite ajustee de la ligne focalisee y 1000..1078 (79 px / 2)"),
    ("screen-key-cap-size", "20px", "options.png : la touche « V » tient dans 41x41 (crop 915,1350,41,41) / 2"),
    ("screen-tile-height", "84px", "main_menu.png : la tuile couvre y 768..934 (167 px / 2)"),
];
...
mod tests {
    ...
    fn chaque_couleur_cite_une_capture_du_manifeste() {
        for c in SCREEN_COLORS {
            assert!(
                crate::screens::CAPTURES.iter().any(|cap| cap.file == c.capture),
    ...
    fn les_bords_ajustes_ont_un_r2_qui_autorise_a_les_citer() {
        for s in SLANT_SAMPLES {
            assert!(s.r2 >= 0.95, "{} : R² {} trop bas pour citer un angle", s.capture, s.r2);
    ...
    fn les_ancres_suivent_la_mesure_reelle_de_nie_aphrody() {
        ...
        let Some(dir) = crate::screens::captures_dir_if_present() else {
            let m = "GOLDEN SAUTE — data/menu absent : les ancres --screen-* ne sont pas re-mesurées";
            eprintln!("{m}");
            println!("{m}");
            return;
        };
        for ancre in [super::PANEL_BODY, super::ROW_FOCUS, super::TILE_DEEP] {
            ...
            let palette = palette_crop(&img, Crop { x, y, w, h }, usize::from(ancre.k))
                .expect("mesure");
            let dominante = &palette[0];
            assert_eq!(dominante.hex, ancre.hex, "{} : la classe dominante a changé", ancre.name);
            ...
            assert!(de < 0.02, "{} : ΔE {de:.4} entre la mesure et la constante", ancre.name);
            assert!(
