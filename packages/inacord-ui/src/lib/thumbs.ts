...
export function useThumbnail(path: string, ext: string, gameDir?: string) {
  ...
  // Un hôte qui ne sait pas produire de vignette (`vignette` absente) est traité comme un
  // format non pris en charge : le composant affiche son icône de repli au lieu d'attendre
  // une image qui ne viendra pas.
  const supporte = THUMB_EXTS.has(ext) && typeof source.vignette === "function";
  ...
  }, [visible, supporte, path, gameDir, source]);
